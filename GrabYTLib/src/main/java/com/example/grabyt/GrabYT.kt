        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use {
                    ...
                    val videoTitleRegex = Regex("""(?<=<meta name="title" content=")(.*)(?="><meta name="description" content=)""")
                    val videoTitle = videoTitleRegex.find(htmlContentInString)?.value

                    val videoViewCountRegex = Regex("""(?<="viewCount":\{"videoViewCountRenderer":\{"viewCount":\{"simpleText":")(.*)(?= views"\},"shortViewCount")""")
                    val videoViewCount = videoViewCountRegex.find(htmlContentInString)?.value?.replace(",", "")

                    val channelNameRegex = Regex("""(?<=<link itemprop="name" content=")(.*)(?="></span><script )""")
                    val channelName = channelNameRegex.find(htmlContentInString)?.value

                    val subscribersCountRegex = Regex("""(?<= subscribers"\}\},"simpleText":")(.*)(?= subscribers"\},"trackingParams":")""")
